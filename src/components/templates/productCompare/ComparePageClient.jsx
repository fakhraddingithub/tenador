...
import ToolCategoryCard, { toolCategoryTitle } from '@/components/common/ToolCategoryCard';
...
export default function ComparePageClient({ initialCategories }) {
    ...
    return (
        <div className="min-h-screen bg-[var(--color-background)] font-sans text-[var(--color-text)] transition-all duration-700 ease-in-out">
            ...
            {/* ارتفاع طبیعی محتوا، بدون هم‌پوشانی با نوبار در موبایل */}
            <div
                className={`w-full max-w-5xl lg:max-w-7xl mx-auto px-4 sm:px-6 ${selectedProducts.length === 0 && !lockedCategory
                    ? 'py-10 sm:py-16 flex flex-col items-center'
                    : 'pt-10 pb-6'
                    }`}
            >
                {selectedProducts.length === 0 && !lockedCategory && (
                    <div className="text-center mb-8 space-y-3 opacity-100 transition-opacity duration-500">
                        <h1 className="text-2xl leading-relaxed sm:text-4xl sm:leading-normal font-bold text-[var(--color-primary)]">مقایسه تخصصی محصولات</h1>
                        <p className="text-neutral-500 leading-7 max-w-2xl mx-auto">
                ...
                {lockedCategory && (
                    <div className="text-center mb-6 space-y-2">
                        <h1 className="text-2xl leading-relaxed sm:text-4xl sm:leading-normal font-bold text-[var(--color-primary)]">
                            مقایسه محصولات {toolCategoryTitle(lockedCategory)}
                        </h1>
                ...
                <div className="relative w-full" ref={searchRef}>
                    <div className="relative group">
                        ...
                        <input
                            type="text"
                            aria-label="جستجوی محصول برای مقایسه"
                            className="block w-full p-4 pr-12 text-base sm:p-5 sm:pr-12 sm:text-lg text-gray-900 border-2 border-neutral-200 rounded-2xl bg-white focus:ring-0 focus:border-[var(--color-primary)] shadow-sm focus:shadow-md transition-all outline-none"
                            placeholder={
                                lockedCategory
                                    ? `جستجوی محصول دیگر در دسته "${toolCategoryTitle(lockedCategory)}"...`
