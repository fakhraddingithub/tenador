...
import { FiArchive, FiCopy, FiEdit3, FiEye, FiFileText, FiPlus, FiRotateCcw, FiSearch, FiSettings, FiTrash2, FiXCircle } from "react-icons/fi";
...
export default function ArticleList() {
  ...
  // کلیدها همان‌هایی‌اند که روت‌های /api/admin/articles و /api/admin/article-cms
  // با آن‌ها گیت شده‌اند؛ «آرشیو» یک PATCH است، پس زیرِ articles.edit می‌رود.
  ...
  const action = async (url, method, success) => {
    ...
    } catch (error) { showError("عملیات انجام نشد", error.message); return null; }
  };
  ...
  const trash = async (id, title) => {
    const confirmed = await Swal.fire({
      title: "انتقال به زباله‌دان؟",
      text: `«${title}» به زباله‌دان می‌رود و بعداً قابل بازیابی است.`,
      icon: "warning",
    ...
    if (confirmed.isConfirmed) await action(`/api/admin/articles/${id}`, "DELETE", "مقاله به زباله‌دان منتقل شد");
  };

  // حذفِ دائمی فقط از زباله‌دان. سرور دوباره چک می‌کند که مقاله واقعاً در
  // زباله‌دان باشد؛ این تأیید صرفاً جلوی کلیکِ اشتباه را می‌گیرد.
  const destroy = async (id, title) => {
    const confirmed = await Swal.fire({
      title: "حذف دائمی مقاله؟",
      text: `«${title}» برای همیشه از دیتابیس حذف می‌شود. این عمل قابل بازگشت نیست.`,
      icon: "warning",
      showCancelButton: true,
    ...
    try {
      ...
      if (!res.ok) throw new Error(result.error || "حذف دائمی انجام نشد");
      await load();
      showSuccess("حذف شد", "مقاله برای همیشه حذف شد.");
  ...
  return <div className="space-y-4">
    <PageHeader title="مدیریت مقالات" subtitle="پیش‌نویس، انتشار، زمان‌بندی و نسخه‌های مقالات تنادور" icon={<FiFileText />} actions={<>{canRoute("/p-admin/admin-articles/settings") ? <Link href="/p-admin/admin-articles/settings"><Button variant="secondary" icon={<FiSettings />}>دسته‌ها و برچسب‌ها</Button></Link> : null}{canCreate ? <Link href="/p-admin/admin-articles/new"><Button icon={<FiPlus />}>مقاله جدید</Button></Link> : null}</>} />
    ...
    <div className="a-card overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-right"><thead><tr className="border-b bg-gray-50" style={{ borderColor: "var(--admin-border)" }}><th className="p-4 text-xs">مقاله</th><th className="p-4 text-xs">دسته</th><th className="p-4 text-xs">وضعیت</th><th className="p-4 text-xs">آخرین تغییر</th><th className="p-4 text-xs">آمار</th><th className="p-4 text-xs w-44">عملیات</th></tr></thead><tbody className="divide-y" style={{ borderColor: "var(--admin-border)" }}>{loading ? <tr><td colSpan={6} className="p-12 text-center text-sm text-gray-400">در حال بارگذاری...</td></tr> : data.articles.map((article) => { const status = STATUS[article.status] || [article.status, "#6b7a70"]; return <tr key={article._id} className="hover:bg-gray-50/70"><td className="p-4"><div className="flex items-center gap-3 min-w-[280px]">{article.cover?.url ? <Image src={article.cover.url} alt="" width={56} height={44} className="h-11 w-14 object-cover" style={{ borderRadius: "var(--admin-radius)" }} /> : <span className="w-14 h-11 bg-gray-100 flex items-center justify-center text-gray-300"><FiFileText /></span>}<div><strong className="block text-sm max-w-sm truncate">{article.title}</strong><span dir="ltr" className="text-[10px] text-gray-400">{article.slug}</span><div className="flex gap-1 mt-1">{article.featured ? <small className="text-[var(--admin-warning)]">ویژه</small> : null}{article.pinned ? <small className="text-[var(--color-primary)]">سنجاق‌شده</small> : null}</div></div></div></td><td className="p-4 text-xs">{article.category?.name || "—"}</td><td className="p-4"><span className="px-2 py-1 text-[11px] font-bold text-white" style={{ background: status[1], borderRadius: "var(--admin-radius)" }}>{view === "trash" ? "زباله‌دان" : status[0]}</span>{article.status === "scheduled" && article.publishedAt ? <small className="block mt-1 text-gray-400">{new Date(article.publishedAt).toLocaleString("fa-IR")}</small> : null}</td><td className="p-4 text-xs text-gray-500">{new Date(article.updatedAt).toLocaleString("fa-IR")}</td><td className="p-4 text-xs"><span>{article.readingTime || 0} دقیقه</span><span className="block text-gray-400">نسخه {article.currentRevision}</span></td><td className="p-4"><div className="flex items-center gap-1">{view === "trash" ? <>{canEdit ? <button onClick={() => action(`/api/admin/article-cms/${article._id}/restore`, "POST", "مقاله بازیابی شد")} title="بازیابی" className="p-2 text-[var(--color-primary)]"><FiRotateCcw /></button> : null}{canDelete ? <button onClick={() => destroy(article._id, article.title)} title="حذف دائمی" className="p-2 text-gray-500 hover:text-red-600"><FiXCircle /></button> : null}</> : <>{canEdit ? <Link href={`/p-admin/admin-articles/${article._id}`} title="ویرایش" className="p-2 text-gray-500 hover:text-[var(--color-primary)]"><FiEdit3 /></Link> : null}{canRoute(`/p-admin/admin-articles/${article._id}/preview`) ? <Link href={`/p-admin/admin-articles/${article._id}/preview`} target="_blank" title="پیش‌نمایش" className="p-2 text-gray-500 hover:text-[var(--color-primary)]"><FiEye /></Link> : null}{canCreate ? <button onClick={() => duplicate(article._id)} title="کپی" className="p-2 text-gray-500 hover:text-[var(--color-primary)]"><FiCopy /></button> : null}{article.status !== "archived" && canEdit ? <button onClick={async () => { try { const res = await fetch(`/api/admin/articles/${article._id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: "archived" }) }); const result = await res.json(); if (!res.ok) throw new Error(result.error); toast.success("Article archived"); await load(); } catch (error) { toast.error(error.message || "Unable to archive article"); } }} title="آرشیو" className="p-2 text-gray-500 hover:text-[var(--admin-warning)]"><FiArchive /></button> : null}{canDelete ? <button onClick={() => trash(article._id, article.title)} title="زباله‌دان" className="p-2 text-gray-500 hover:text-red-600"><FiTrash2 /></button> : null}</>}</div></td></tr>; })}{!loading && data.articles.length === 0 ? <tr><td colSpan={6} className="p-14 text-center text-sm text-gray-400">مقاله‌ای در این بخش وجود ندارد.</td></tr> : null}</tbody></table></div></div>
