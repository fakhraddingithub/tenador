...
export default function ModernSerieAIFlow({ brandId, brandName }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // دریافت همزمان اطلاعات برند و سری‌های آن
        const [brandRes, seriesRes] = await Promise.all([
          fetch(`/api/brands/${brandId}`),
          fetch(`/api/series?brand=${brandId}`), // فرض بر این است که این API لیست سری‌های برند را می‌دهد
        ]);
  ...
  const generatePrompt = () => {
    if (rawText.length < 20)
      return toast.warning("لطفاً متن کامل‌تری وارد کنید");
    setLoading(true);

    setTimeout(() => {
      const prompt = buildSerieTemplate({
        brand,
        existingSeries: series, // لیست سری‌های موجود را اینجا بفرستید
        rawContent: rawText,
      });
      setAiPrompt(prompt);
      setStep(2);
  ...
  const validateAndParse = () => {
    try {
      ...
      toast.error(
        "فرمت JSON وارد شده معتبر نیست. لطفاً دقیقاً همان چیزی که AI داده را پیست کنید.",
      );
    }
  };
    ...
    return (
      <div className="max-w-4xl mx-auto my-20 text-center space-y-4">
        ...
        <p className="font-bold text-gray-400">
          در حال فراخوانی دستورالعمل‌های برند...
        </p>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto my-10 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center px-12 relative">
        ...
        {steps.map((s) => (
          ...
          >
            <div
              className={`w-14 h-14 rounded-[1.5rem] flex items-center justify-center transition-all duration-500 shadow-xl ${
                step >= s.id
            ...
            <span
              className={`text-xs font-bold tracking-tight ${step >= s.id ? "text-black" : "text-gray-300"}`}
            >
              {s.label}
            </span>
          </div>
      ...
      <div className="bg-white rounded-[3rem] border border-gray-100 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.08)] overflow-hidden min-h-[500px] flex flex-col">
          ...
          <div className="p-10 flex-1 flex flex-col space-y-6 animate-in slide-in-from-left-5 duration-500">
            <div className="space-y-1 text-right">
              <h2 className="text-2xl font-bold italic flex flex-row-reverse items-center gap-3">
                <FaPenNib className="text-white bg-black p-2 rounded-xl" />{" "}
                استخراج هوشمند اطلاعات
              </h2>
              <p className="text-gray-400 text-sm font-bold">
                توضیحات یا کاتالوگ سری جدید محصول را در کادر زیر وارد کنید.
              </p>
            </div>
            ...
            >
              {loading ? (
                ...
                <>
                  <FaMagic className="text-[var(--color-primary)]" /> ساخت دستور
                  هوشمند
                </>
              )}
            </button>
        ...
        {step === 2 && (
          <div className="p-10 flex-1 flex flex-col space-y-6 animate-in zoom-in-95 duration-500">
            <div className="bg-[var(--color-primary)]/10 border-r-4 border-[var(--color-primary)] p-6 rounded-2xl text-right">
              ...
              <p className="text-sm text-gray-700 font-bold leading-6">
                دستور زیر را کپی کرده و به هوش مصنوعی (مانند ChatGPT) بدهید. سپس
                خروجی کد (JSON) را کپی کرده و در مرحله بعد وارد کنید.
              </p>
            </div>
            ...
            <div className="flex gap-4">
              <button
                onClick={() => setStep(1)}
                className="flex-1 py-5 font-bold text-xs text-gray-400 hover:text-black transition-all"
              >
                بازگشت به مرحله قبل
              </button>
              <button
        ...
        {step === 3 && (
          <div className="p-10 flex-1 flex flex-col space-y-6 animate-in slide-in-from-right-5 duration-500">
            <div className="space-y-1 text-right">
              <h2 className="text-2xl font-bold italic flex flex-row-reverse items-center gap-3">
                <FaCode className="text-white bg-black p-2 rounded-xl" />{" "}
                بارگذاری خروجی هوش مصنوعی
              </h2>
              <p className="text-gray-400 text-sm font-bold">
                کد JSON دریافت شده را در کادر زیر قرار دهید.
              </p>
            </div>
